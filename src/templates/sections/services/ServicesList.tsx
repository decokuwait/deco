          ...
          {s.items.map((it, i) => (
            <li key={it.id} className="border-b border-line">
              <article className="group grid grid-cols-[auto_1fr] items-center gap-x-4 gap-y-3 py-6 transition-all duration-300 hover:ps-3 sm:grid-cols-[4.5rem_auto_1fr_auto] sm:gap-x-6 sm:py-7">
                <span className="font-heading text-3xl font-black leading-none text-primary/25 tabular-nums transition-colors duration-300 group-hover:text-accent sm:text-5xl">
                  {String(i + 1).padStart(2, "0")}
                </span>
                <span className="flex h-12 w-12 items-center justify-center rounded-full bg-primary/10 text-primary ring-1 ring-primary/15 transition duration-300 group-hover:bg-primary group-hover:text-primary-fg sm:h-14 sm:w-14">
