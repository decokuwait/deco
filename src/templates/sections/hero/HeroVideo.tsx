  ...
  return (
    <section className="tone-dark relative flex min-h-[82svh] items-end overflow-hidden bg-secondary text-white">
      <div className="absolute inset-0">
        {h.videoUrl ? <HeroAutoVideo src={h.videoUrl} poster={poster || undefined} className="h-full w-full object-cover" /> : <Img src={poster} alt={heroAlt(ctx)} sizes={SIZES.full} ratio="16/9" className="h-full w-full object-cover" eager />}
      </div>
      ...
      <Container wide className="relative pb-14 pt-32 sm:pb-20">
        <div className="max-w-2xl animate-fade-up">
          ...
          <h1 className={cx("mt-4 font-heading text-4xl font-black leading-[1.1] sm:text-6xl", headingLeading(ctx))}>{heroTitle(ctx)}</h1>
