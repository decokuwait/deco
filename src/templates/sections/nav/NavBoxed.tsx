...
export function NavBoxed({ ctx }: SectionProps) {
  ...
  return (
    <>
      <TopAnchor />
      <header className={cx("sticky z-50 bg-bg pt-2 sm:pt-3", CHROME_TOP)}>
        <Container wide>
          <div className="flex h-16 items-center justify-between gap-3 rounded-card border border-line border-b-4 border-b-accent bg-surface px-3 shadow-sm sm:h-20 sm:gap-4 sm:px-5">
            <Brand ctx={ctx} />
