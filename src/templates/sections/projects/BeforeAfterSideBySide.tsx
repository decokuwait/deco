...
function Pane({ item, alt, label, tone }: { item: MediaItem | null; alt: string; label: string; tone: "before" | "after" }) {
  return (
    <div className="relative aspect-[4/3] overflow-hidden bg-surface-2">
      {item && <Media item={item} alt={alt} sizes={SIZES.half} className="h-full w-full object-cover" />}
            ...
            return (
              <article key={pr.id} className="overflow-hidden rounded-card border border-line bg-surface">
                <div className="relative grid gap-1 sm:grid-cols-2">
                  <Pane item={before} alt={`${ctx.ui("before")} — ${before ? mediaAlt(ctx, before, pr) : projectAlt(ctx, pr)}`} label={ctx.ui("before")} tone="before" />
                  <Pane item={after} alt={`${ctx.ui("after")} — ${after ? mediaAlt(ctx, after, pr) : projectAlt(ctx, pr)}`} label={ctx.ui("after")} tone="after" />
